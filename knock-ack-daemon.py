...
from knockknock.KnockWatcherClient import KnockWatcherClient

import knockknock.daemonize

def checkPrivileges():
    if (not os.geteuid() == 0):
        print "Sorry, you have to run knock-ack-daemon as root."
        sys.exit(3)
...
def handleService():
     print "Received knock-ack from server. Proceed with service request."
...
def handleKnocks(output, profiles, config):
    ...
    knockWatcherClient = KnockWatcherClient(config, logFile, profiles, portOpener)

    knockWatcherClient.tailAndProcess()
...
def main(argv):
    ...
    if (profiles.isEmpty()):
        print 'WARNING: Running knock-ack-daemon without any active profiles.'
